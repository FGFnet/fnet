    ...
    return (
      <Collapse in={alertOpen} sx={{position: 'fixed', top:'30vh', left:'30vw', zIndex: 50}}>
        <Alert
          ...
          action={
            <Box>
                <Button onClick={()=>{setAlertOpen(false); deleteTodo();}}>삭제</Button>
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        />
        { !props.todo && <IconButton color="primary" onClick={addTodo}><CheckIcon /></IconButton> }
        { props.todo && <IconButton color="primary" onClick={()=>setAlertOpen(true)}><DeleteIcon /></IconButton> }
      </Box>
    </React.Fragment>
  )
}
