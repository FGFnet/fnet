...
function FGSection({ fg }: { fg: FG | null }) {
  ...
  const lcCount = useQuery('getMyLCCountregister', async () => await UserService.getLCMemberCount(String(todayLC.data.data[0].name), token), {
          ...
          {!todayLC.isLoading && todayLC.data && todayLC.data.data !== null && todayLC.data?.data.length > 0 &&
           todayLC.data.data.map((item:any) => (
            <Box key={item.id}>
              ...
              {!lcCount.isLoading &&
                <div style={{ marginBottom: 20 }}>{todayLC.data.data.name} 접수 인원 : {lcCount.data.data.register}</div>
              }
              <Button variant="outlined" style={{ margin: 'auto' }} onClick={() => navigate(`/lc/${item.name}`)}>
